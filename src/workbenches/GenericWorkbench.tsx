...
export function GenericWorkbench({ engine }: { readonly engine: TomlEngine }) {
  ...
  const setEditorTheme = (next: RainglowThemeId) => { setThemeId(next); saveEditorTheme(next); };
  ...
  const validate = useCallback(async () => {
    ...
    const notices: Diagnostic[] = response.notices.map((notice) => ({ ...rangeFromOffsets(sourceRef.current, 0, Math.min(1, sourceRef.current.length)), severity: notice.severity, source: "schema", ruleId: notice.ruleId, message: notice.message, explanation: notice.explanation, suggestion: "Declare the intended JSON Schema draft explicitly if the default is not correct." }));
    ...
    setStatus({ type: errors ? "invalid" : "valid", message: errors ? `${errors} blocking ${errors === 1 ? "error" : "errors"} and ${next.length - errors} non-blocking findings.` : `Valid against ${primary.fileName}.` });
  ...
  const formatSource = async () => { try { const formatted = await adapters[format].formatSource(sourceRef.current, FORMAT_OPTIONS); updateSource(formatted); await validate(); } catch (cause) { setStatus({ type: "error", message: cause instanceof Error ? cause.message : String(cause) }); } };
